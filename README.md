...
## **THIS COOKBOOK IS DEPRECATED**python Cookbook
===============

[![Build Status](https://travis-ci.org/poise/python.png?branch=master)](https://travis-ci.org/poise/python)

Installs and configures Python.  Also includes LWRPs for managing python packages with `pip` and `virtual` isolated Python environments.
...
- :install: Install a pip package - if a version is provided, install that specific version (default)
- :upgrade: Upgrade a pip package - if a version is provided, upgrade to that specific version
...
- version: the version of the package to install/upgrade.  If no version is given the latest is assumed.
...
- interpreter: The Python interpreter to use. the default is null (i.e. use whatever python the virtualenv command is using).
